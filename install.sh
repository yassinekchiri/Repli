...
# directory and the whole virtualenv unreadable to anyone but root — the
# service account could then not even spawn the interpreter (systemd reports
# "status=203/EXEC, Permission denied"). Everything secret is created under
# an explicit umask 077 / chmod later on; the rest must be world-readable.
umask 022
...
Options:
  ...
  --python PATH       base interpreter for the venv (default: newest system
                      Python 3.9+ the service account can actually reach)
