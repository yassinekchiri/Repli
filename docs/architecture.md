...
| `test` | cascade complete and **healthy on all three legs**; **no test environment already in place**; qtrees exist on the source and are unique; derived clone name is a legal ONTAP volume name; peering, plus the **clone mirror's own** policy `MFA_MirrorAllSnapshots` and schedule `pg-15-minutely` visible on the DR cluster |
...
| `test` / `clone` volume settings | states what every clone will be set to (encryption, snapshot reserve, space guarantee, snapshot policy, security style, volume export policy); warns that a **`unix` clone cannot take AD-group DACLs**, so action `acl` will refuse it |
| `test` / `clone` quotas | the **source** tree rules are readable; **preview of both rules** each clone volume will get in the `default` policy — the volume-level rule at 0 B and the qtree rule with the source's limits; warns when the source qtree has **no quota** (the destination qtree gets none either — `unlimited` is not `0`) |
